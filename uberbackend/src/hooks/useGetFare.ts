...
export const useGetFare = async (distanceTime: DistanceTime) => {
    const baseFare = {
        auto: 30,
        car: 50,
        moto: 20
    ...
    // Validate and process the distance (in meters)
    let distanceValue = distanceTime.distance; // Already in meters

    // Validate and process the duration (in minutes)
    let durationValue = distanceTime.duration; // Already in minutes

    // Calculate fare based on vehicle type and distance/time
    const fare = {
        AUTO: Math.round(baseFare.auto + ((distanceValue / 1000) * perKmRate.auto) + (durationValue * perMinuteRate.auto)),
        CAR: Math.round(baseFare.car + ((distanceValue / 1000) * perKmRate.car) + (durationValue * perMinuteRate.car)),
        BIKE: Math.round(baseFare.moto + ((distanceValue / 1000) * perKmRate.moto) + (durationValue * perMinuteRate.moto))
    };
    return fare;
};
